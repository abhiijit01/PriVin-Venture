    ...
    public class PageController : Controller
    {
        ...
        public JsonResult Index(Page thepg)
        {
            try
            {
                if (thepg.pageid >= 1)
                {
                    ReturnValue = BLL.PageManagement.GetInstance.UpdatePage(thepg);
                }
                else
                {
                    ReturnValue = BLL.PageManagement.GetInstance.InsertPage(thepg);
                }
        ...
        public JsonResult DeletePage(string[] Finid)
        {
            ...
            try
            {
                DataTable data = BLL.PageManagement.GetInstance.DeletePage(Fin_Id);
        ...
        {
            DataTable dt = new DataTable();
            dt = BLL.PageManagement.GetInstance.GetPageById(x);
            try
            {
        ...
        {
            DataTable dt = BLL.PageManagement.GetInstance.GetPageList();
            try
            {
                List<Page> Pagelist = new List<Page>();
