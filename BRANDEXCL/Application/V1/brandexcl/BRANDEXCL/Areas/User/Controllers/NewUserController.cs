    ...
    public class NewUserController : Controller
    {
        ...
        public JsonResult CreateUser(NewUser theUser)
        {
            try
            {
                ...
                if (theUser.Usr_Id >= 1)
                {
                      ReturnValue = BLL.NewUserBs.GetInstance.UpdateUser(theUser);
                }
                else
                {
                    ReturnValue = BLL.NewUserBs.GetInstance.CreateUser(theUser);
                }
                if (ReturnValue >= 1)
                {
        ...
        public JsonResult ChangePassword(NewUser theUser)
        {
            try
            {
                ...
                ReturnValue = BLL.NewUserBs.GetInstance.ChangePwd(theUser);
        ...
        public JsonResult ForgotPassword(NewUser theUser)
        {
            try
            {
                ReturnValue = BLL.NewUserBs.GetInstance.CheckExist(theUser);
                if (ReturnValue == 1)
                {
