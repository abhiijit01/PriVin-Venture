...
namespace BRANDEXCL.Areas.Outlet.Controllers
{
    public class ReceivedProductController : Controller
    {
        ...
        {
            int RetVal = 0;
            RetVal = BRANDEXCL.Business.ReceivedProductManagement.GetInstance.UpdateReceivedProductqty(barcode, sizeid, qty);
            return Json(RetVal);
        }
        public JsonResult BarcodeSelectByBarcode(string barcode, int? sizeid)
        {
            DataTable dt = new DataTable();
            dt = BRANDEXCL.Business.BarcodeManagement.GetInstance.BarcodeSelectByBarcode(barcode, sizeid);
            ...
            return Json(ProductRecieveList);
        }
        ...
        public JsonResult UpdateReceivedProduct(ReceivedProduct theUser)
        {
            try
            {
                ReturnValue = BRANDEXCL.Business.ReceivedProductManagement.GetInstance.UpdateReceivedProduct(theUser);
        ...
        public ActionResult GenerateBarCodeAndGetBarcode()
        {
            ...
            {
                bm.Save(memoryStream, System.Drawing.Imaging.ImageFormat.Jpeg);
                return Json(new { Barcode = RndBarcode, BarcodeImage = "data:image/png;base64," + Convert.ToBase64String(memoryStream.ToArray()) });
            }
        }
